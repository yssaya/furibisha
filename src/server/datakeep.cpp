...
int nRookHandicap[ROOK_HANDICAP_NUM];

const char ROOK_PROB_SYN[] = "./handicap/rook_prob.txt";
int nRookProb[ROOK_PROB_NUM];

const char KLD_SYN[] = "./handicap/kld.txt";
int nKLD[KLD_NUM];
...
*/
	{
		...
		if ( fp==NULL ) die(ERR_INT("fopen AVERAGE_WINRATE ERROR"));
		char str[256];
	...
	{
		FILE *fp = fopen(ROOK_HANDICAP_SYN,"r");
		if ( fp==NULL ) die(ERR_INT("fopen ROOK_HANDICAP_SYN ERROR"));
		for (int i=0;i<ROOK_HANDICAP_NUM;i++) {
			char str[256];
			if ( fgets( str, 255, fp ) == NULL ) die(ERR_INT("ROOK_HANDICAP_SYN i=%d ERROR",i));
			nRookHandicap[i] = atoi(str);
		}
		fclose(fp);
	}
	{
		FILE *fp = fopen(ROOK_PROB_SYN,"r");
		if ( fp==NULL ) die(ERR_INT("fopen ROOK_PROB_SYN ERROR"));
		for (int i=0;i<ROOK_PROB_NUM;i++) {
			char str[256];
			if ( fgets( str, 255, fp ) == NULL ) die(ERR_INT("ROOK_PROB_SYN i=%d ERROR",i));
			nRookProb[i] = atoi(str);
		}
		fclose(fp);
	}

	{
		FILE *fp = fopen(KLD_SYN,"r");
		if ( fp==NULL ) die(ERR_INT("fopen KLD_SYN ERROR"));
		for (int i=0;i<KLD_NUM;i++) {
			char str[256];
			if ( fgets( str, 255, fp ) == NULL ) die(ERR_INT("KLD_SYN i=%d ERROR",i));
			nKLD[i] = atoi(str);
		}
		fclose(fp);
	}
