...
float Client::receive_header(const OSI::Conn &conn, uint TO, uint bufsiz) {
  ...
  assert(HEADER_SIZE >= 1+1+2+HANDICAP_TYPE*2+2 +2*3+2+ROOK_PROB_NUM*2+ROOK_HANDICAP_NUM*2);
...
//  cout << "handi rate " << i << " = " << x << endl;
  ...
  char *p = buf + 4 + HANDICAP_TYPE*2 + 2;
  _kld_playout  = bytes_to_int<uint16_t>(p+0);
  _kld_gain     = bytes_to_int<uint16_t>(p+2);
  _kld_interval = bytes_to_int<uint16_t>(p+4);
  _mtemp        = bytes_to_int<uint16_t>(p+6);
  p += 4*2;
  for (int i=0; i<ROOK_PROB_NUM; i++) {
    uint16_t x = bytes_to_int<uint16_t>(p + i*2);
    _rook_file_prob[i] = x;
  }
  p += ROOK_PROB_NUM*2;
  for (int i=0; i<ROOK_HANDICAP_NUM; i++) {
    uint16_t x = bytes_to_int<uint16_t>(p + i*2);
    _rook_handicap[i] = (short)x;
  }
