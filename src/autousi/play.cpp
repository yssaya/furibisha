...
class USIEngine : public Child {
  ...
  string _record_wght, _record_version, _record_settings, _record_handicap, _record_furibisha;
  int64_t _id_wght;
...
//  char opt_kld_value[] = "0.0000045";
    char opt_kld_value[] = "0.00000175";
...
//  char opt_kld_i_value[] = "100";  // check kldgain each 100 playouts. base 800/8 = 100, 800*4 = 3200, -p 3200 -kldgain 0.000013 -kldinterval 100
//  char opt_kld_i_value[] = "200";  // -p 6400 -kldgain 0.0000045 -kldinterval 200
    char opt_kld_i_value[] = "400";  // -p 12800 -kldgain 0.00000175 -kldinterval 400 
    argv[argc++] = opt_kld_i;
...
//    else                              _flag_do_resign = true;
    _flag_do_resign = false;  // 20240908
...
*/
    ...
    _record_handicap = string(", FuriHandicap ");
    _record_handicap += buf;
    engine_out("setoption name USI_FuriHandicapRate value %s", buf);

    int fpos[18] = {0};
    const int F99 = 81+9*2;
	if ( F99+8 != ROOK_PROB_NUM ) die(ERR_INT("F99+8 != ROOK_PROB_NUM"));
    const uint *pr = Client::get().get_rook_file_prob();
    int sum = 0;
    for (int i=0; i<F99; i++) sum += pr[i];
    if ( sum <= 0 ) die(ERR_INT("invalid rook_file_prob sum=%d", sum));
    unsigned int r = mt_rand();
    int ri = (r % sum) + 1;
    int i,s = 0;
    for (i=0; i<F99; i++) {
      s += pr[i];
    ...
    if ( i==F99 ) die(ERR_INT("not found ri=%d,sum=%d", ri,sum));
    int fs = -1;
    int fg = -1;
    if ( i<81 ) {
      fs = i / 9;
      ...
      int any_sum = 0;
      for (int i=F99; i<F99+8; i++) any_sum += pr[i];
      if ( any_sum <= 0 ) die(ERR_INT("invalid rook_file_prob any_sum=%d", any_sum));
      unsigned int r = mt_rand();
      int ri = (r % any_sum) + 1;
      int i,s = 0;
      for (i=F99; i<F99+8; i++) {
      ...
      if ( i==F99+8 ) die(ERR_INT("not found ri=%d,any_sum=%d", ri,any_sum));
      int select_n = i - F99 + 2;	// from 2 to 9

      int c[9]; // copy to reduce probability
      if ( fs >= 0 ) for (int i=0;i<9;i++) c[i] = pr[fs*9+i];
      if ( fg >= 0 ) for (int i=0;i<9;i++) c[i] = pr[i*9+fg];
      if ( (fs < 0 && fg < 0) || (fs >= 0 && fg >= 0) ) die(ERR_INT("err fs or fg"));

      int select_sum = 0;
      for (int i=0; i<9; i++) select_sum += c[i];
	  for (int k=0; k<select_n; k++) {
        if ( select_sum <= 0 ) die(ERR_INT("invalid selct_sum=%d,k=%d", select_sum,k));
        unsigned int r = mt_rand();
        int ri = (r % select_sum) + 1;
        int i,s=0;
        for (i=0;i<9;i++) {
          s += c[i];
          if ( s >= ri ) break;
        }
        if ( i==9 ) die(ERR_INT("not found ri=%d,select_sum=%d,k=%d", ri,select_sum,k));
        select_sum -= c[i];
        c[i] = 0;
        fpos[i+(fs>=0)*9] = 1;
      }
    ...
    buf[9] = ':';
    buf[19]=0;
    _record_furibisha = string(", FuriPos ");
    _record_furibisha += buf;
    engine_out("setoption name USI_FuriPos value %s", buf);	// "00000010:000001000"	sente ibisha, gote shikenbisha
#endif
...
//    if (value < th_resign && _nmove > 30) flag_resign = true;
    ...
    if (flag_resign && _flag_do_resign) {
      ...
      rec += string("'") + _record_settings + _record_handicap + _record_furibisha + string("\n");
      rec += _record_main;
